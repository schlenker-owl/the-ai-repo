...
from airoad.sft.pref_trainers import build_dpo_trainer
from transformers import AutoModelForCausalLM, AutoTokenizer, EarlyStoppingCallback

"""
Slim DPO training CLI for Qwen-0.5B (LoRA/DoRA compatible).

- Loads DPO pairs (prompt, chosen, rejected)
- Applies LoRA/DoRA to the student; uses **frozen base** as reference
- TRL DPOTrainer with time-limit + live logging; optional eval
"""
...
def main() -> None:
    ...
    ap.add_argument("--config", type=str, default="configs/slm/qwen05b_dpo_fast_plain.yaml")
    args = ap.parse_args()
    ...
    # datasets (DPO expects 'prompt','chosen','rejected')
    ...
    if loss_cb.rows:
        ...
        with open(out_dir / "train_log.csv", "w", newline="") as f:
            ...
            w = _csv.DictWriter(f, fieldnames=sorted({k for r in loss_cb.rows for k in r.keys()}))
            w.writeheader()
