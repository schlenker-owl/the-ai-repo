...
@app.command()
def main(n: int = 200, d: int = 1, noise: float = 0.1, lr: float = 0.1, epochs: int = 500, standardize_x: bool = True):
    ...
    typer.echo(f"True w[:3]: {w_true[:min(3,len(w_true))]}  |  Learned w[:3]: {model.w[:min(3,len(model.w))]}")
