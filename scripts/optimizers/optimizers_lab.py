...
def run(opt_name: str, steps: int, lr: float):
    ...
    d = X.shape[1]
    w = np.zeros((d, 1)); b = 0.0
    if opt_name == "sgd": opt = SGD(lr=lr)
    elif opt_name == "momentum": opt = Momentum(lr=lr)
    elif opt_name == "adam": opt = Adam(lr=lr)
    else: raise ValueError("opt_name ∈ {sgd,momentum,adam}")
