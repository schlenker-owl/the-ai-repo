...
$$
- \(Q,K,V \in \mathbb{R}^{T \times d_k}\) are queries/keys/values per head.  
- \(M\) is the **causal mask** with \(-\infty\) above the main diagonal to prevent looking ahead.
...
$$
X_\text{input} = \text{TokenEmbed}(x) + \text{PosEmbed}(t)
$$

## Generation
At inference, feed the current context (last `block_size` tokens), compute logits, sample next token, append, and repeat.
...
* Compare dropout 0.0 vs 0.1 to see regularization effect.

## Pitfalls

* Mask shape mismatches → ensure broadcast shape `(1,1,T,T)` and slice `:T,:T`.
* Sequence length > `block_size` → assert and/or crop context window.
