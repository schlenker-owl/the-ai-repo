...
## Intuition
Predict the **next character** given the previous context. A vanilla RNN maintains a hidden state; **LSTM/GRU** add gating to reduce vanishing gradients and to carry information longer.
...
* Increase `--block-size` (context length) and watch loss behavior.
* Try LR sweeps (`1e-3`, `3e-3`, `1e-2`) and note stability.

## Pitfalls

* **Exploding gradients** → add clipping: `torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)`.
* **Long sequences on CPU/MPS** → grow batch size gradually; monitor memory.
